    ...
    <section className="py-24 bg-gradient-to-b from-slate-950 to-slate-900">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-10 items-start">
          {/* Traditional */}
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 opacity-70">
            ...
            <div className="space-y-2">
              {traditionalSteps.map((s, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 py-2.5 px-3 rounded-lg bg-slate-800/40 border border-slate-700/40 text-sm text-slate-400">
                    {s.label}
                  </div>
                </div>
              ))}
          ...
          {/* OmniBid */}
          <div className="bg-slate-900 border border-amber-500/20 rounded-2xl p-6 shadow-xl shadow-amber-500/5">
                ...
                <div key={i} className="flex items-center gap-3">
                  {!s.sub && (
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                      s.highlight
                        ...
                        : 'bg-slate-800 border border-amber-500/30 text-amber-500'
                    }`}>
                      {i + 1}
                    </div>
                  )}
                  ...
                  <div className={`flex-1 py-2.5 px-3 rounded-lg text-sm font-medium ${
                    s.highlight
                      ...
                      : s.sub
                      ? 'bg-slate-800/30 border border-dashed border-slate-700/60 text-slate-500 text-xs italic'

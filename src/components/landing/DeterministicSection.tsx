      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-3">
            {hardRules.map((r) => (
              <div key={r.trigger} className="bg-slate-900 border border-slate-800 rounded-xl p-4 group hover:border-rose-500/30 transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-1">
                    <div className="text-sm font-semibold text-white">{r.trigger}</div>
                    <div className="text-xs text-slate-500 font-mono">{r.example}</div>
                  </div>
            ...
            <div className="bg-slate-900 border border-emerald-500/20 rounded-xl p-4">
              <div className="text-sm font-semibold text-emerald-400 mb-1">All other opportunities</div>
              <div className="text-xs text-slate-400">Pass through full multi-agent AI analysis → APPLY / MAYBE / SKIP</div>
            </div>
          </div>
        </div>
      </div>
